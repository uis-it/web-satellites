...
class CMS_Controller_Plugin_Acl extends Zend_Controller_Plugin_Abstract
{
    ...
    public function preDispatch(Zend_Controller_Request_Abstract $request)
    {
		...
		$acl->addRole(new Zend_Acl_Role('clearcache'), 'user');
		$acl->addRole(new Zend_Acl_Role('webmeeting'), 'user');
		...
		$acl->add(new Zend_Acl_Resource('meeting'));
		$acl->add(new Zend_Acl_Resource('forum'));
		$acl->add(new Zend_Acl_Resource('skins'));
		$acl->add(new Zend_Acl_Resource('externals'));
		$acl->add(new Zend_Acl_Resource('cache'));
		$acl->add(new Zend_Acl_Resource('konkurranse'));
		$acl->add(new Zend_Acl_Resource('favicon.ico'));
		...
		// meeting
		$acl->allow('user', 'meeting', array('adminqa','list'));
		
		// clear-cache
		$acl->allow('user', 'cache', array('list','delete'));
